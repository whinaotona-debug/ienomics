...
function composeExplain(about, pct) {
  ...
  if (about === '日経平均') {
    ...
    return {
      ...
      body: [
        ...
        '学習のポイントは「なぜ動いたか」の仮説を立てることと、それが翌日も有効かは別問題だと知ること。短期の値動きに一喜一憂するより、金利・為替・海外市場との関係を追うほうが、12〜15歳の学びになる。参考リンクは公式発表で、見出しの転載ではない。'
      ].join('\n\n')
    };
  }
  if (about === 'S&P500') {
    ...
    return {
      ...
      body: [
        ...
        '中央銀行の金利や、企業開示のルールに関する発表は、お金の借りやすさやリスクの取り方を変えることがある。原文を全部読む必要はない。「金融政策か、規制か、個別の処分か」と種類を分けるだけで十分だ。参考は公式発表へのリンクだけにする。'
      ].join('\n\n')
    };
  }
  if (about === '金') {
  ...
  return {
    ...
    body: [
      ...
      '産油地域の情勢、OPECプラスの方針、在庫発表などが材料になる。一日の下落を「これから安い」と決めない。エネルギーは国境を越えて取引される、という前提で公式発表を読む。'
    ].join('\n\n')
  };
}
