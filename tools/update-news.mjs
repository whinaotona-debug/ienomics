...
function composeExplain(about, pct) {
  ...
  if (about === '日経平均') {
    ...
    return {
      ...
      body: [
        `きょうのトピックは日経平均だ。東京の証券取引所に上場する代表企業の株価を平均した指数で、${d === 'up' ? '直近は上がった' : d === 'down' ? '直近は下がった' : '直近はほぼ横ばいだった'}。${abs ? `変化は約${abs}%だ。` : ''}買う人と売る人のどちらが多いかが、この数字に表れる。`,
  ...
  if (about === 'S&P500') {
    ...
    return {
      ...
      body: [
        `きょうのトピックはS&P500だ。米国の主要約500社でつくる指数で、${d === 'up' ? '直近は上がった' : d === 'down' ? '直近は下がった' : '直近はほぼ横ばいだった'}。${abs ? `変化は約${abs}%だ。` : ''}世界の投資の基準になりやすい。`,
  ...
  if (about === '金') {
    ...
    return {
      ...
      body: [
        `きょうのトピックは金（ゴールド）だ。${d === 'up' ? '直近は上がった' : d === 'down' ? '直近は下がった' : '直近はほぼ変わらなかった'}。${abs ? `変化は約${abs}%だ。` : ''}企業の利益ではなく、通貨の価値や金利、先行きの不安で動きやすい。`,
  ...
  return {
    ...
    body: [
      `きょうのトピックは原油だ。${d === 'up' ? '直近は上がった' : d === 'down' ? '直近は下がった' : '直近はほぼ横ばいだった'}。${abs ? `変化は約${abs}%だ。` : ''}輸送燃料や化学製品の原料になるので、物価や企業のコストの話題と一緒に語られる。`,
