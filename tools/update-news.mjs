...
function composeExplain(about, pct) {
  ...
  if (about === '日経平均') {
    if (d === 'up') {
      return {
        title: '日経平均は直近、上昇した',
        body: '日経平均株価は、日本の代表的な企業の株価を平均した指数だ。買う人が売る人より多いと上がる。今日の動きは「日本株全体のムード」の一面で、翌日に逆転することもある。一つの数字だけで将来を決めないのが基本だ。'
      };
    }
    if (d === 'down') {
      return {
        title: '日経平均は直近、下落した',
        body: '株価は企業の業績だけでなく、金利や海外市場、投資家の慎重さでも動く。下がった日は売りが優勢だった、という記録に近い。トレンドかどうかは、数日〜数週間の流れと材料をセットで見る。'
      };
    }
    return {
      title: '日経平均は直近、ほぼ横ばいだった',
      body: '指数が動かない日は、買いと売りが拮抗していることが多い。大きな材料が出ていない、という読みもある。静かな日も市場の普通の状態だ。'
    };
  }
  if (about === 'S&P500') {
    if (d === 'up') {
      return {
        title: 'S&P500は直近、上昇した',
        body: 'S&P500は米国の主要約500社で構成する株価指数で、世界の投資の基準になりやすい。金利見通しや企業決算が材料になることが多い。米国株の動きは、日本株にも波及することがある。'
      };
    }
    if (d === 'down') {
      return {
        title: 'S&P500は直近、下落した',
        body: '遠い市場でも、価格は需給で決まる。下落は、リスクを取りにくくなったサインのことがある。ただし一日の下落が「景気の終わり」を意味するとは限らない。'
      };
    }
    return {
      title: 'S&P500は直近、ほぼ横ばいだった',
      body: '指数が動かないときは、新しい材料が少なかったか、強気と弱気が打ち消し合っていることが多い。米国は金利と雇用の指標が注目されやすい。'
    };
  }
  if (about === '金') {
    if (d === 'up') {
      return {
        title: '金価格は直近、上昇した',
        body: '金（ゴールド）は企業の利益では動かず、通貨の価値や金利、地政学リスクの影響を受けやすい。金利が低い、または先行きが不透明だと買われやすい、という説明がよく使われる。'
      };
    }
    if (d === 'down') {
      return {
        title: '金価格は直近、下落した',
        body: '金が下がる局面では、株式などリスク資産のほうに資金が向きやすいことがある。逆も起こる。貴金属の価格は「安全資産」というラベルだけで決めつけないほうがいい。'
      };
    }
    return {
      title: '金価格は直近、ほぼ変わらなかった',
      body: '金は実物資産で、配当はない。動きが小さい日は、為替や金利の材料が一段落していることが多い。株価指数とは別物として比較すると理解しやすい。'
    };
  }
  if (d === 'up') {
    return {
      title: '原油価格は直近、上昇した',
      body: '原油は輸送燃料や化学製品の原料になる。需要が増える、供給が絞られる、と見られると上がりやすい。燃料コストは物価にもつながりうるので、株や為替と一緒に語られる。'
    };
  }
  if (d === 'down') {
    return {
      title: '原油価格は直近、下落した',
      body: '原油安は輸送コストの低下につながることもあるが、需要減のサインと読まれることもある。産油国の方針や在庫統計で翌日にひっくり返りやすい。'
    };
  }
  return {
    title: '原油価格は直近、ほぼ横ばいだった',
    body: '需給のバランスが一時的に安定していると、価格の変動は小さくなる。エネルギー価格は世界の出来事で急変しやすいので、横ばいも「今は材料待ち」と見ることができる。'
  };
}

function composeExplainPack(about, pct) {
  const main = composeExplain(about, pct);
  const extra = {
    日経平均: [
      {
        title: '日経平均は「日本株の代表指数」',
        body: '個別企業ではなく、選ばれた銘柄の平均だから、市場全体の温度を見るのに使う。構成銘柄は入れ替わるし、値がさ株の影響が強い、という特徴もある。ニュースでは指数そのものと、個別企業の話を分けて読むと混乱しにくい。'
      },
      {
        title: '一日の騰落だけでは戦略は決まらない',
        body: '学習のポイントは「なぜ動いたか」の仮説を立てることと、それが翌日も有効かは別問題だと知ること。短期の値動きに一喜一憂するより、金利・為替・海外市場との関係を追うほうが中学生以上の学びになる。'
      }
    ],
    'S&P500': [
      {
        title: 'S&P500は米国経済の縮図として見られる',
        body: '時価総額の大きい企業が指数に与える影響が大きい。ITや金融の比重が変わるだけで、指数の意味合いも変わる。日本から見るときは、円ドル相場とセットで考えると実感しやすい。'
      },
      {
        title: '米当局の発表は、世界の資金の流れを動かす',
        body: '中央銀行の金利や、企業開示のルールに関する発表は、借りやすさやリスクの取り方を変えることがある。原文を全部読む必要はない。「金融政策か、規制か、個別の処分か」と種類を分けるだけで十分だ。'
      }
    ],
    金: [
      {
        title: '金は株式と違う値動きをしやすい',
        body: '株は将来の利益の見込みで価格がつく。金は実物で、金利が高いと「利息のつかない金」は相対的に不利、と説明されることが多い。だから金と株を同時に見るときは、金利の方向を意識する。'
      },
      {
        title: '見出しの「金」はゴールドとは限らない',
        body: '金額・資金・金利は金属の金ではない。学びでは金価格（ゴールド）だけをこの欄に載せる。単語だけで分類すると誤るので、本文が実物資産の話かを確認する習慣が役に立つ。'
      }
    ],
    原油: [
      {
        title: '原油はエネルギーと物価の接続点',
        body: 'ガソリン、航空燃料、プラスチックなどの上流にある。需要（景気や移動）と供給（産出・在庫・輸出規制）がずれると価格が動く。家計では「運ぶコスト」、企業では「原材料コスト」として効いてくる。'
      },
      {
        title: '原油ニュースは地政学と統計が材料になりやすい',
        body: '産油地域の情勢、OPECプラスの方針、在庫発表などが材料になる。一日の下落を「これから安い」と決めない。エネルギーは国境を越えて取引される、という前提で読む。'
      }
    ]
  };
...
async function main() {
  ...
  const items = ['日経平均', 'S&P500', '金', '原油'].flatMap(about => {
    const pack = composeExplainPack(about, moves[about]);
