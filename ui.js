...
function renderHeader() {
  ...
  if (state.role === 'parent') {
    ...
    nameTag = `<p class="text-[10px] text-slate-300 font-semibold tracking-[0.1em]">${state.childName} の${rb('資産','しさん')}</p>`;
  }
...
function renderBank() {
  ...
  return `
    <h2 class="text-lg font-bold mb-4 border-b border-slate-100 pb-3 text-slate-800 flex items-center gap-2">
      <div class="w-4 h-4 text-emerald-500">${getIcon('bank')}</div>${rb('家庭内銀行','かていないぎんこう')}
    </h2>
  ...
  return `
    ...
    ${state.role === 'child' ? `
      <div class="flex gap-2 mb-6">
        <input type="number" id="invest-amount" placeholder="金額" class="flex-1 p-3 bg-white border border-slate-200 rounded-xl font-bold text-sm focus:outline-none" />
        ...
        return `
          <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-col gap-2">
            ...
            <div class="flex justify-between items-center text-[10px] font-bold text-slate-400">
              ...
              ${state.role === 'child' ? `<button onclick="sellCustom('${inv.id}', ${val})" class="text-slate-500 hover:text-slate-800 bg-white px-3 py-1.5 rounded border border-slate-200">売却する</button>` : ''}
            </div>
          </div>
        `;
  ...
  return `
    <div class="h-full flex flex-col items-center justify-center p-6 bg-slate-50 relative overflow-hidden">
      ...
      <div class="w-full max-w-sm bg-white p-8 rounded-3xl shadow-xl border border-slate-100 relative z-10 text-center animate-in zoom-in-95">
        ...
        <p class="text-[10px] font-bold text-slate-500 mb-6 leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-100">次回以降のログインに使う<br>パスワードを決めてください。</p>
...
function renderWaitingChild() {
  return `
    <div class="h-full flex flex-col items-center justify-center p-6 bg-slate-50 relative overflow-hidden">
      <img src="logo.png" class="absolute inset-0 w-full h-full object-cover opacity-5 pointer-events-none mix-blend-multiply" onerror="this.style.display='none'" />
      <div class="w-full max-w-sm bg-white p-8 rounded-3xl shadow-xl border border-slate-100 relative z-10 text-center animate-in zoom-in-95">
        <h3 class="font-black text-slate-800 mb-2 text-lg">${state.childName} の連携待機中</h3>
        <p class="text-[10px] font-bold text-slate-500 mb-6 leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-100">子供の端末で「子供として開始」を選び、<br>以下の同期IDを入力してください。</p>
        <div class="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl mb-6 font-mono font-black text-3xl tracking-widest text-slate-800">${state.familyCode}</div>
        <div class="flex items-center justify-center gap-2 mb-6 text-xs font-bold text-slate-400 animate-pulse"><div class="w-4 h-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin"></div>子供の接続を待機中...</div>
        ${state.children.length > 1 ? `<button onclick="switchActiveChild('${state.children.find(c=>c.id!==state.familyCode)?.id}')" class="text-[10px] text-blue-500 font-bold mb-4">別の子供の画面へ</button><br>` : ''}
