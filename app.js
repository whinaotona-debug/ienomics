...
import { render } from './ui.js?v=187';
import { applyFuriganaState, requestPushPermission, sendPushNotification, getTemplateIdFromTask, dateKeyToValue, getCurrentMarketRates, japanTodayKey, japanParts, japanDeadlineMs, msUntilJapanMidnight, marketNameFromId, MARKET_META, getInvestmentPortfolioValue, getHoldingValue, getHoldingShares, getInvestmentValues, getActiveInvestments, normalizeSheetUrl, parseMarketSheetCsv, setMarketSheetSeries, scheduledPaymentAmount } from './utils.js?v=187';
import { showAlert, showConfirm, showPrompt, showToast, setBusy } from './dialog.js?v=187';
import { startTutorial, hasSeenTutorial } from './tutorial.js?v=187';
import { initPush, isPushActive, isPushSupported, requestPushPermission as askPushPermission, unregisterPush, getPushError } from './push.js?v=187';
import { db, auth } from './firebase.js?v=187';
...
window.saveStockCap = async () => {
  ...
  await guard('saveStockCap', async () => {
    await updateDoc(doc(db, "families", state.familyCode), {
      stockCap: cap == null ? deleteField() : cap,
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js?v=187').catch(err => console.warn('SW登録失敗:', err));
  });
}