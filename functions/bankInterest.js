/** Sync with ../bankInterest.js — keep algorithms identical. */
/**
...
const BANK_DAILY_DIVISOR = 30;
const BANK_INTEREST_ENGINE_VERSION = 2;
...
function bankJapanTodayKey(date = new Date()) {
  const j = japanParts(date);
  return `${j.year}-${j.month}-${j.day}`;
}

function bankJapanMonthKey(date = new Date()) {
  const j = japanParts(date);
  return `${j.year}-${j.month}`;
}

function bankMonthKeyNum(key) {
  const [y, m] = String(key || '').split('-').map(Number);
  if (!y || !m) return 0;
  return y * 12 + m;
}

function bankNextMonthKey(key) {
  const [y, m] = String(key || '').split('-').map(Number);
...
function bankDayKeyNum(key) {
  const [y, m, d] = String(key || '').split('-').map(Number);
  if (!y || !m || !d) return 0;
  return y * 10000 + m * 100 + d;
}

function bankMonthKeyFromDayKey(dayKey) {
...
function bankLastDayOfMonthKey(monthKey) {
  const [y, m] = String(monthKey || '').split('-').map(Number);
  if (!y || !m) return '';
  return `${y}-${m}-${lastDayOfCalendarMonth(y, m)}`;
}

function bankPrevDayKey(dayKey) {
...
function bankNextDayKey(dayKey) {
  const [y, m, d] = String(dayKey || '').split('-').map(Number);
  if (!y || !m || !d) return '';
  const ms = new Date(`${y}-${pad2(m)}-${pad2(d)}T12:00:00+09:00`).getTime() + 86400000;
  return bankJapanTodayKey(new Date(ms));
 ...
 */
function initialBankDepositFields(amount, createdAtMs, now = Date.now()) {
 ...
 * @param {Date|number} [now]
 */
function migrateBankDepositIfNeeded(doc, now = Date.now()) {
  const nowDate = now instanceof Date ? now : new Date(now);
 ...
 */
function computeBankInterestState(doc, now = Date.now()) {
  let todayKey;
...
/** Firestore に書く差分があるか */
function bankInterestStateChanged(before, after) {
...
function bankInterestWritePayload(state) {
  return {
    amount: state.amount,
...
function selfTestBankInterestLogic() {
...
module.exports = {
  ...
  bankMonthBefore,
  bankDayKeyNum,
  bankMonthKeyFromDayKey,
  bankLastDayOfMonthKey,
  bankPrevDayKey,
  bankNextDayKey,
  bankDailyInterest,
  bankDisplayBalance,
  bankDisplayInterest,
  bankDisplayPrincipal,
  initialBankDepositFields,
  migrateBankDepositIfNeeded,
