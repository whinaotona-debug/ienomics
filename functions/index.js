...
exports.generateRepeatedTasks = onSchedule(
  ...
  async () => {
    const now = new Date();
    // Asia/Tokyo 指定のスケジューラなので、ここは日本時間の壁時計でよい
    ...
    for (const d of snap.docs) {
      ...
      let due = false;
      if (temp.type === 'weekly') due = days.includes(day);
      else if (temp.type === 'monthly') due = days.includes(date);
      if (!due) continue;
      ...
      deadline.setHours(Number.isFinite(hours) ? hours : 19, minutes, 0, 0);
    ...
    console.log(`[generateRepeatedTasks] 作成${created} / 既存スキップ${skipped} (${todayStr})`);
  }
);
